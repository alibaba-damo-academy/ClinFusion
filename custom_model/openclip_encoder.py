#################################################################################################################################################################################
#           Modified version of https://github.com/dvlab-research/MGM/blob/769820cb71afc7c2b9a2166ca3ce92db1636215c/mgm/model/multimodal_encoder/openclip_encoder.py.           #
#################################################################################################################################################################################
...
class OpenCLIPVisionTower(nn.Module):
    ...
    ### Modified
    def __init__(self, vision_tower, model_path, force_image_size, delay_load=False):
        super().__init__()

        self.is_loaded = False
        self.vision_tower_name = vision_tower
        self.model_path = model_path
        self.vision_config = json.load(open(os.path.join(model_path,'open_clip_config.json'), 'r'))
        self.vision_config["model_cfg"]["vision_cfg"]["image_size"] = force_image_size
        self.is_droppath = True 
    ...
    def load_model(self):
        ...
        if 'convnext' in self.vision_tower_name:
            ...
            self.vision_config["model_cfg"]["hidden_sizes"] = self.model_channel

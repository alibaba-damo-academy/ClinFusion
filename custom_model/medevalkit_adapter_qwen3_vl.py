...
from .modeling_multi_enc_qwen3_vl import MultiEnc_Qwen3VLForConditionalGeneration, MultiEnc_Qwen3VLModel
...
def _ct_preprocessing_flexible(
    ...
    if isinstance(path_or_object, str):
        ...
        transforms = Compose([
            ...
            ScaleIntensityRanged(keys=["image"], a_min=intensity_window[0], a_max=intensity_window[1], b_min=0.0,
                                 b_max=1.0, clip=True),
...
class DummyArgs:
    ...
    def __init__(self,
        ...
        self.new_encoder_3_type = new_encoder_3_type
        self.new_encoder_3_path = new_encoder_3_path
...
def get_config_values(
    ...
    if not config_file.is_file():
        config_file_in_dir = config_file / "config.json"
        if (config_file.name != "config.json") and config_file_in_dir.is_file():
            config_file = config_file_in_dir
        else:
            raise FileNotFoundError(f"Config file not found at: {config_file_in_dir}")
...
class MedEvalKitAdapter:
    ...
    def __init__(self, model_path: str, model_config: dict, generation_config: dict):
        ...
        original_qwen3_vl_path = {
            False: "cache/models/Qwen3-VL-8B-Instruct",
            True: "cache/models/Qwen3-VL-32B-Instruct",
        }["32b" in model_path]
        original_new_encoder_path = {
            "dinov2": "cache/models/dinov2-large",
            "convnext": "cache/models/CLIP-convnext_large_d_320.laion2B-s29B-b131K-ft-soup",
            "pe_3d": "",
            # "pe_3d": "/mnt/eff_nas/hanyizeng/code/Med-CLIP3D/logs/pe_v3aug/120k_4datasets_window_1000_1000/0x0_0x0_0x0/model_SSL_large_p8_3d_pe-lr_5e-05-bsz_8-grad_accum_1-epochs_2000-loss_clip-label_smoothing_0x0-logit_scale_up_100x0-workers_8-precision_amp_bf16-2025_10_24-23_16_59/checkpoints/epoch_100.pt",
        }
        self.llm = MultiEnc_Qwen3VLForConditionalGeneration.from_pretrained(
            original_qwen3_vl_path,
            torch_dtype="auto",
        ...
        new_encoder_types = [one_type for one_type in config_values["vision_config.vision_model_config"].keys() if
                             one_type != "original_enc"]
        print(f"Leading additional encoders: {new_encoder_types}")
        ...
        model_args = DummyArgs(
            ...
            new_encoder_3_type=None if len(new_encoder_types) <= 2 else new_encoder_types[2],
            new_encoder_3_path=None if len(new_encoder_types) <= 2 else original_new_encoder_path[new_encoder_types[2]],
            compute_dtype="auto",
            multi_enc_fusion_type=config_values["vision_config.fusion_type"],
            convnext_image_size=config_values[
                "vision_config.vision_model_config.convnext.model_cfg.vision_cfg.image_size"],
        )
        additional_processor_values = get_config_values(os.path.join(model_path, "preprocessor_config.json"),
                                                        ["merlin_patch_size"])
        model_args.update(additional_processor_values)

        vl_model = MultiEnc_Qwen3VLModel.from_qwen_model(self.llm.model)
        vl_model = add_new_enc_to_model(vl_model, model_args)
        self.llm.set_model(vl_model)

        # --- 3. Load the fine-tuned model weights (sharded checkpoint) ---
        print(f"Start loading model from checkpoint: {model_path}")
        load_info = load_sharded_checkpoint(self.llm, model_path, strict=False, prefer_safe=True)
        print(f"Finish loading model from checkpoint. \nLoad info: {load_info}")
        self.llm.eval()

        # --- 4. Initialize the custom multi-modal processor ---
        init_kwargs = {'trust_remote_code': False, 'cache_dir': None, 'revision': 'main', 'token': None}
        processor = AutoProcessor.from_pretrained(original_qwen3_vl_path, **init_kwargs)
        ...
        additional_processor_values = get_config_values(
            ...
            [
                ...
                "merlin_sample_images"
            ]
        )
        additional_processor_values[
            "CT_volume_cache"] = "cache/3d_volume"
        model_args.update(additional_processor_values)
    ...
    def generate(self, batch_data: List[Dict[str, Any]]) -> List[str]:
        ...
        for item in batch_data:
            ...
            ### Convert images to nifti, so that we can test on M3D dataset.
            img_num_threshold = 32  # self.processor.image_processor.merlin_volume_depth
            if len(image_inputs) >= img_num_threshold * .75:
                image_inputs, nifti_inputs = self.transform_images_to_one_nifti(image_inputs, nifti_inputs, self.processor.image_processor.merlin_volume_depth)
            ...
            # For Zero-image inference
            # pil_images = []; volume = []; print("We do not use visual input for inference.")
        ...
        ### Support Text+2D mixed inference
        if all_images[0] is not None and all_volumes[0] is not None:
            ...
            all_images = [i for i in all_images if i is not None]
            if all_images:
                inputs = self.processor(
                    text=prompts,
                    images=all_images,
    ...
    def _pre_sample_slices_from_volume(self, inputs, processor):
        ...
        for item in inputs:
            ...
            if one_volume is not None:
                ...
                new_images = sample_slices_from_volume_to_pil_fast(
                    one_volume,
                    num_slices=processor.image_processor.merlin_sample_images,
                    random=False,
                )
    ...
    def transform_images_to_one_nifti(
            self,
            image_inputs: List[Image.Image],
            nifti_inputs: List[Any],
            num_frames_in_volume: int
    ) -> (List[Any], List[Any]):
        """
        Stacks a list of PIL.Image objects into a single 3D NIfTI volume of a fixed depth.

        This function takes a list of 2D images and ensures the final volume has a
        depth equal to `num_frames_in_volume`.
        - If more images are provided, it uses the last `num_frames_in_volume`.
        - If fewer images are provided, it repeats the last image to pad the volume.

        Args:
            image_inputs (List[Image.Image]): A list of PIL.Image objects.
            nifti_inputs (List[Any]): A list of existing NIfTI inputs.
            num_frames_in_volume (int): The desired number of slices (depth) in the
                                        output NIfTI volume.

        Returns:
            Tuple[List[Any], List[Any]]: A tuple containing:
                - An empty list (as all images are consumed into the NIfTI).
                - The updated list of NIfTI inputs, including the new volume.
        """
        # If there are no images to process, do nothing.
        if not image_inputs:
            return [], nifti_inputs

        # --- NEW LOGIC: Truncate or prepare for padding ---
        num_images = len(image_inputs)
        if num_images > num_frames_in_volume:
            print(f"Info: Received {num_images} images, but volume depth is {num_frames_in_volume}. "
                  f"Using the last {num_frames_in_volume} images.")
            image_inputs = image_inputs[-num_frames_in_volume:]

        print(f"Processing {len(image_inputs)} images to create a NIfTI volume of depth {num_frames_in_volume}...")
        ...
        if isinstance(image_inputs[0], Image.Image):
            ...
            print(f"Warning: First item in image_inputs is not a PIL.Image. Cannot determine target size.")
            return image_inputs, nifti_inputs

        for i, pil_image in enumerate(image_inputs):
            if not isinstance(pil_image, Image.Image):
                print(f"Warning: Item {i} is not a PIL.Image, skipping.")
                continue

            if pil_image.size != target_size:
                print(f"Warning: Image {i} has size {pil_image.size}, resizing to {target_size}.")
                pil_image = pil_image.resize(target_size, Image.Resampling.LANCZOS)

            if pil_image.mode != 'L':
                pil_image_gray = pil_image.convert("L")
            else:
                pil_image_gray = pil_image

            slices_np.append(np.array(pil_image_gray))

        if not slices_np:
            print("Warning: No valid PIL images were found to create a NIfTI volume.")
            return [], nifti_inputs

        # --- NEW LOGIC: Pad the volume by repeating the last frame ---
        num_current_slices = len(slices_np)
        if num_current_slices < num_frames_in_volume:
            num_to_pad = num_frames_in_volume - num_current_slices
            print(f"Info: Have {num_current_slices} slices, need {num_frames_in_volume}. "
                  f"Repeating the last slice {num_to_pad} times.")

            last_slice = slices_np[-1]
            for _ in range(num_to_pad):
                slices_np.append(last_slice)

        try:
            # Stack the list of 2D arrays (H, W) into a single 3D array (H, W, D).
            volume_data = np.stack(slices_np, axis=-1).astype(np.float32)

            affine = np.eye(4)
            nifti_volume = nib.Nifti1Image(volume_data, affine)

            print(f"Successfully created a NIfTI volume with shape: {volume_data.shape}")

            updated_nifti_inputs = list(nifti_inputs)
            updated_nifti_inputs.append(nifti_volume)

            return [], updated_nifti_inputs

        except Exception as e:
            print(f"Error: Failed to stack images into a NIfTI volume: {e}")
            return image_inputs, nifti_inputs

...
def apply_rotary_emb(freqs, t, start_index=0, scale=1.0, seq_dim=-2, mask=None):
    dtype = t.dtype
    ...
    ###FIXME: for grad checkpoint
    # print(t.shape, freqs.shape, mask.shape)
    if freqs.shape[seq_dim] == 64 * 64 * 64:
        _, _, n, c = freqs.shape
        m = round(n ** (1/3))
        l = round(t.shape[seq_dim] ** (1/3))
        freqs = freqs.reshape(1, 1, m,m,m, c)
        _freqs = freqs[:, :, :l, :l, :l, :].reshape(1, 1, l*l*l, c).contiguous()
        
        if mask is not None:
            _freqs = freqs.repeat(mask.shape[0], 1, 1, 1)
            _freqs = _freqs[mask.unsqueeze(1) == 0]
            b, _, nn, cc = _freqs.shape
            _freqs = _freqs[mask.unsqueeze(1) == 0].reshape(b, 1, -1, cc)

        t = (t * _freqs.cos() * scale) + (rotate_half(t) * _freqs.sin() * scale)
    else:
        if mask is not None:
            _freqs = freqs.repeat(mask.shape[0], 1, 1, 1)
            b, _, nn, cc = _freqs.shape
...
class RotaryEmbedding(Module):
    def __init__(
        ...
        self.freqs = nn.Parameter(freqs, requires_grad=learned_freq).to(device=device)
        # print(self.freqs.device, self.freqs.shape)
    ...
    def forward(self, t: Tensor, seq_len=None, offset=0):
        ...
        # print(t.device, freqs.device, t.dtype, freqs.dtype, t.shape, freqs.shape)
...
class Rope3D:
    ...
    def __init__(self, dim, device='cuda', use_cls_token=False):
        self.dim = dim
        self.use_cls_token = use_cls_token
        self.grid_size = None
        self.freq = None
    ...
    def init_tensors(self):
        self.rope = RotaryEmbedding(self.dim // 3, device=self.device)
        self.rope_z = RotaryEmbedding(self.dim - self.dim//6*4, device=self.device)

    def update_grid(self, device, grid_z, grid_h, grid_w):
        if self.grid_size != (grid_z, grid_h, grid_w):
            self.grid_size = (grid_z, grid_h, grid_w)

            # self.rope = self.rope.to(device)

            if self.use_cls_token:
                # +1 to leave space for the cls token to be (0, 0, 0)
                grid_z_range = torch.arange(grid_z, device=device) + 1
                grid_y_range = torch.arange(grid_h, device=device) + 1
                grid_x_range = torch.arange(grid_w, device=device) + 1
            else:
                grid_z_range = torch.arange(grid_z, device=device)
                grid_y_range = torch.arange(grid_h, device=device)
                grid_x_range = torch.arange(grid_w, device=device)

            freqs_z = self.rope_z(grid_z_range)[:, None, None].expand(grid_z, grid_h, grid_w, -1)
            freqs_y = self.rope(grid_y_range)[None, :, None].expand(grid_z, grid_h, grid_w, -1)
            freqs_x = self.rope(grid_x_range)[None, None, :].expand(grid_z, grid_h, grid_w, -1)
            freq = torch.cat([freqs_x, freqs_y, freqs_z], dim=-1).reshape(grid_h * grid_w * grid_z, -1)
    ...
    def __call__(self, q, k, mask=None):
        # batch, heads, seq, dim = q.shape
        q = apply_rotary_emb(self.freq[:, None, :, :], q, mask=mask)
        k = apply_rotary_emb(self.freq[:, None, :, :], k, mask=mask)

...
class VisionTransformer(nn.Module):
    ...
    def _sample_abs_posemb(self, grid_h: int, grid_w: int, grid_d: int):
        """Interpolates the absolute position embedding if necessary."""
        if self.posemb_grid_size == grid_h and self.posemb_grid_size == grid_w:
            if self.use_cls_token:
                cls_token_embed = self.positional_embedding[0:1, :]  # [1,C]
                # [1,HW,C]->[D,HW,C]->[DHW,C]
                pos_embed = self.positional_embedding[None, 1:, :].repeat(grid_d, 1, 1).reshape(-1, self.width)
                pos_embed = torch.cat([cls_token_embed, pos_embed], dim=0)  # [1+DHW,C]
            else:
                # [1,HW,C]->[D,HW,C]->[DHW,C]
                pos_embed = self.positional_embedding[None, :, :].repeat(grid_d, 1, 1).reshape(-1, self.width)
        ...
        # [HW,C]->[D,HW,C]->[DHW,C]
        pos_embed = pos_embed[None].repeat(grid_d, 1, 1).reshape(-1, self.width)
    ...
    def forward_features_3d(
        ...
        grid_d = d // self.depth_patch_size
        ...
        if self.use_abs_posemb:
            x = x + self._sample_abs_posemb(grid_h, grid_w, grid_d)

        if self.use_rope3d:
            self.rope.update_grid(x.device, grid_h, grid_w, grid_d)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        x = self.forward_features_3d(x, norm=True, **kwargs)
        x = self._pool(x)
...
class MyCLIP(nn.Module):
    def __init__(
        ...
        self.image_size = self.visual.image_size  # For ease of use
        self.text_encoder.pooler = BertPooler(text_encoder.config.hidden_size, vision_cfg.output_dim) # we always reset the pooling layer
        self.logit_scale = nn.Parameter(torch.ones([]) * init_logit_scale)
    ...
    def encode_text(self, input_ids, token_type_ids, attention_mask):
        x = self.text_encoder(input_ids=input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)
        return x[1]
    ...
    def forward(self, image, text):
        ...
        text_features = (
            self.encode_text(**text) if text is not None else None
        )
        return image_features, text_features, self.logit_scale

class MyCLIP_ssl(nn.Module):
    def __init__(
        self,
        vision_encoder,
        text_encoder,
    ):
        super(MyCLIP_ssl, self).__init__()
        ...
        if self.text_encoder is not None:
            self.text_encoder.pooler = BertPooler(text_encoder.config.hidden_size, vision_encoder.output_dim) # we always reset the pooling layer
    ...
    def encode_image(self, image, normalize: bool = False, retain_thw = False):
        x = self.visual(image)
        ...
        if not retain_thw:
            ...
            latent_t, latent_h, latent_w = t // patch_size, h // patch_size, w // patch_size
            x = x.reshape(b, latent_t, latent_h, latent_w, -1).permute(0, 4, 1, 2, 3)
            # [4, 2048, 1024] -> [4, 1024, 8, 16, 16] 
            return x

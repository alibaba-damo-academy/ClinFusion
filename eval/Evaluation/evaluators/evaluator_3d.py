...
class Evaluator:
    ...
    # --- Private Methods for 'report_generation' ---

    def _prepare_report_generation_prompt(self, data: dict) -> dict:
        """Handles prompt creation for report generation."""
        
        assert "ct_path" in data, "Input data missing 'ct_path' key."
    ...
    def _prepare_mcq_prompt(self, data: dict) -> dict:
        ...
        if "ct_path" in data and data["ct_path"]:
            start_str = "Answer the following multiple-choice question based on the provided 3D CT scan.\n\n"
        else:
    ...
    **Incorrect Format Examples (THESE WILL FAIL AUTOMATED GRADING):**
    ...
    def _prepare_open_prompt(self, data: dict) -> dict:
        ...
        if "ct_path" not in data:
            start_str = "Analyze the following question and provide a concise, direct answer.\n\n"
        else:
            start_str = "Analyze the following 3D CT scan and provide a concise, direct answer to the following question.\n\n"
